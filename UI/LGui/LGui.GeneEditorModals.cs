...
{
    private void OpenLGuiGeneItemEditor(float? restoredScrollPosition = null)
    {
        ...
        Action rebuild = () => OpenLGuiGeneItemEditor(CaptureLGuiModalScrollPosition(editorScroll));
        var y = 4f;
        AddLGuiInlineInput(content, T("源ID", "Source ID"), () => _geneEditorSourceId, value => _geneEditorSourceId = value, 0f, y, 120f, 200f);
        CreateLGuiButton(content, "SourceTable", T("源ID对应表", "Source ID table"), 340f, y, 170f, 42f, () =>
        {
            var scrollPosition = CaptureLGuiModalScrollPosition(editorScroll);
            OpenLGuiGeneSourceReference(() => OpenLGuiGeneItemEditor(scrollPosition));
        });
        y += 52f;
        ...
        y += 60f;
        y = AddLGuiEffectEditor(content, T("基因效果", "Gene effects"), _geneEditorValues, GetGeneEffectName, null, y, rebuild, () =>
        {
            var scrollPosition = CaptureLGuiModalScrollPosition(editorScroll);
            OpenLGuiEffectReference(T("基因效果对应表", "Gene effect table"), GetFilteredGeneEffectIds, () => _geneEffectFilter, value => _geneEffectFilter = value, _geneEffectPage, value => _geneEffectPage = value, row => _geneEditorValues.Add(new GeneValueInput(row.Id.ToString(CultureInfo.InvariantCulture), "0")), () => OpenLGuiGeneItemEditor(scrollPosition));
        });
        y += 10f;
        CreateLGuiButton(content, "Apply", T("确认", "Confirm"), 0f, y, 120f, 44f, () => ApplyLGuiGeneDataChange(CaptureLGuiModalScrollPosition(editorScroll)));
    ...
    private void OpenLGuiGeneSourceReference(Action returnEditor, float? restoredScrollPosition = null)
    {
        ...
        CreateLGuiButton(content, "Search", T("搜索", "Search"), 434f, y, 100f, 44f, () => { _geneSourcePage = 0; OpenLGuiGeneSourceReference(returnEditor, CaptureLGuiModalScrollPosition(referenceScroll)); });
        ...
        y = BuildLGuiReferencePager(content, rows.Count, _geneSourcePage, y, next => { _geneSourcePage = next; OpenLGuiGeneSourceReference(returnEditor, CaptureLGuiModalScrollPosition(referenceScroll)); });
        ...
        for (var i = start; i < end; i++)
        {
            var row = rows[i];
            var local = row;
            CreateLGuiButton(content, "Source" + i, row.DisplayName + " | " + row.Id, 0f, y, 1280f, 44f, () => { _geneEditorSourceId = local.Id; returnEditor(); });
            y += 48f;
        }
    ...
    private void ApplyLGuiItemDataChange(float? restoredScrollPosition = null)
    {
        ...
        if (completed) CloseLGuiEditorModal(); else OpenLGuiItemDataEditor(restoredScrollPosition);
    }
    private void ApplyLGuiFoodDataChange(float? restoredScrollPosition = null)
    {
        _foodEditorWindowVisible = true;
        ApplyFoodEditorChange();
        var completed = !_foodEditorWindowVisible;
        _foodEditorWindowVisible = false;
        if (completed) CloseLGuiEditorModal(); else OpenLGuiFoodEditor(restoredScrollPosition);
    }
    private void ApplyLGuiWeaponDataChange(float? restoredScrollPosition = null)
    {
        ...
        if (completed) CloseLGuiEditorModal(); else OpenLGuiWeaponEditor(restoredScrollPosition);
    }
    private void ApplyLGuiGeneDataChange(float? restoredScrollPosition = null)
    {
        _geneEditorWindowVisible = true;
        ApplyGeneEditorChange();
        var completed = !_geneEditorWindowVisible;
        _geneEditorWindowVisible = false;
        if (completed) CloseLGuiEditorModal(); else OpenLGuiGeneItemEditor(restoredScrollPosition);
    }
}
