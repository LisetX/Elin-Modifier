...
public sealed partial class ElinModifierPlugin
{
    private void BindLGuiFeatureRow(RectTransform rect, LGuiFeatureRow model, int index)
    {
        ...
        view.Favorite.gameObject.SetActive(true);
        var isFavorite = IsIndependentFeatureFavorite(model.Id);
        view.FavoriteText.text = isFavorite ? "★" : "☆";
        var favoriteColor = isFavorite ? new Color(1f, 0.72f, 0.12f, 1f) : GetActiveUiTextColor();
        var favoriteColors = view.Favorite.colors;
        favoriteColors.normalColor = favoriteColor;
        favoriteColors.highlightedColor = Color.Lerp(favoriteColor, Color.white, 0.28f);
        favoriteColors.pressedColor = Color.Lerp(favoriteColor, Color.black, 0.18f);
        favoriteColors.selectedColor = favoriteColor;
        favoriteColors.disabledColor = new Color(favoriteColor.r, favoriteColor.g, favoriteColor.b, 0.45f);
        favoriteColors.colorMultiplier = 1f;
        view.Favorite.colors = favoriteColors;
        view.FavoriteText.color = favoriteColor;
