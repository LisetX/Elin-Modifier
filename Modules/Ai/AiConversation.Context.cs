...
public sealed partial class ElinModifierPlugin
{
    ...
    private static string BuildAiSystemPrompt(bool includeTools)
    {
        ...
        if (includeTools)
            prompt += " You may use EMG tools, including high-risk runtime reflection, arbitrary runtime method invocation, and Harmony patch tools whenever built-in EMG tools do not directly complete the user's request. " +
                      ...
                      " For knowledge questions about the game itself, prefer the read-only query tools over guessing or over runtime reflection: query_item for what an item is, query_obtain for how to get something (including items with no recipe), query_uses for what a material is good for, query_recipe_tree for full build planning against the current backpack, query_npc, query_element, query_material, and query_player_state for the player snapshot. These tools read game data directly and change nothing, so use them freely and answer from their output instead of from memory." +
                      " For overworld questions - where a town or dungeon is, what is nearby, where an item or NPC can be found on the world map, or how to travel somewhere - use the world map tools: world_map_overview for the region snapshot, world_map_zones to list zones with coordinates and distance, world_map_search for a fuzzy keyword search over terrain, biomes, gatherable and minable products, prebuilt town contents and known NPCs, and world_map_route to plan a road-preferring route and set the on-screen route guide. These read the live world and change nothing except the route guide destination." +
    ...
    private static void AppendAiToolDefinitionsJson(StringBuilder sb)
    {
        ...
        sb.Append(",");
        AppendAiToolDefinition(sb, "query_item", "Read full game-data detail for one item: category, tags, level, value, weight, quality, default material, crafting materials, required workbench, dismantle yields, enchantment slots and description. Use this when the user asks what an item is or what it is made of.", "item",
            "\"item\":{\"type\":\"string\",\"description\":\"Item ID, exact display name, or partial display name.\"}");
        sb.Append(",");
        AppendAiToolDefinition(sb, "query_obtain", "Answer how to obtain an item. Aggregates every acquisition path found in game data: crafting materials and workbench, the recipe table, dismantling other items, gathering from map objects, NPC loot and corpse components, spawn lists, and shop/gift/delivery category flags. Use this whenever the user asks where something comes from or how to get it, including items that have no recipe.", "item",
            "\"item\":{\"type\":\"string\",\"description\":\"Item ID, exact display name, or partial display name.\"},\"limit\":{\"type\":\"integer\",\"description\":\"Maximum rows per section. Default 20.\"}");
        sb.Append(",");
        AppendAiToolDefinition(sb, "query_uses", "List everything that can be crafted from an item, plus what the item itself yields when dismantled. Use this when the user asks what a material is good for.", "item",
            "\"item\":{\"type\":\"string\",\"description\":\"Item ID, exact display name, or partial display name.\"},\"limit\":{\"type\":\"integer\",\"description\":\"Maximum rows. Default 40.\"}");
        sb.Append(",");
        AppendAiToolDefinition(sb, "query_recipe_tree", "Expand the full crafting material tree for an item, recursively, and compare it against the player's current backpack. Reports need/have/missing per node and a flat shopping list of the raw materials still required. Use this for build planning questions.", "item",
            "\"item\":{\"type\":\"string\",\"description\":\"Item ID, exact display name, or partial display name.\"},\"count\":{\"type\":\"integer\",\"description\":\"How many to craft. Default 1.\"},\"depth\":{\"type\":\"integer\",\"description\":\"Maximum recursion depth, 1-8. Default 4.\"},\"check_inventory\":{\"type\":\"boolean\",\"description\":\"Subtract what the player already carries. Default true.\"}");
        sb.Append(",");
        AppendAiToolDefinition(sb, "query_npc", "Read full game-data detail for one NPC: race, job, level, hostility, biome, tags, traits, tactics, faith, works, hobbies, elements, combat acts, equipment, recruit items, loot table and the spawn lists it appears in.", "npc",
            "\"npc\":{\"type\":\"string\",\"description\":\"NPC ID, exact display name, or partial display name.\"}");
        sb.Append(",");
        AppendAiToolDefinition(sb, "query_element", "Read full game-data detail for one skill, feat, spell, ability or enchantment: type, group, category, target, proc, cost, requirements, max level, gene slot, cooldown, charge, radius, trainer info, food effect and description. Also reports the player's current value.", "element",
            "\"element\":{\"type\":\"string\",\"description\":\"Element numeric ID, alias, exact display name, or partial display name.\"}");
        sb.Append(",");
        AppendAiToolDefinition(sb, "query_material", "Read material properties: hardness, tags, tier, quality, value and which items use it as their default material. Hardness drives gathering and mining thresholds.", "material",
            "\"material\":{\"type\":\"string\",\"description\":\"Material alias, exact display name, or partial display name.\"}");
        sb.Append(",");
        AppendAiToolDefinition(sb, "world_map_overview", "Read the world map overview: region name, grid size and origin, terrain composition, road cell count, the player's world grid position, zone counts, and the current route-guide destination. Use this before answering questions about the overworld.", "",
            "");
        sb.Append(",");
        AppendAiToolDefinition(sb, "world_map_zones", "List zones placed on the world map with their grid coordinates, type, level, danger level, discovery flags and Chebyshev distance from the player. Use this to answer where a town or dungeon is, or what is nearby.", "",
            "\"filter\":{\"type\":\"string\",\"description\":\"Optional zone display name or zone class name filter.\"},\"category\":{\"type\":\"string\",\"enum\":[\"all\",\"landmark\",\"dungeon\",\"field\",\"site\",\"owned\"],\"description\":\"Optional category filter. Default all.\"},\"known_only\":{\"type\":\"boolean\",\"description\":\"Only zones the player has discovered. Default false.\"},\"limit\":{\"type\":\"integer\",\"description\":\"Maximum rows, capped at 200. Use 0 for the cap. Default 30.\"}");
        sb.Append(",");
        AppendAiToolDefinition(sb, "world_map_search", "Search the world map for a keyword and return matching locations with grid coordinates. Covers terrain tiles and biomes, zone names and tags, gatherable and minable products per biome, fixed furniture and fixed NPCs inside prebuilt town/landmark maps, and NPCs in zones the mod already has data for. Matching is fuzzy and multi-keyword. Use this to answer where something can be found on the overworld.", "query",
            "\"query\":{\"type\":\"string\",\"description\":\"Keyword, or several keywords separated by spaces; all keywords must match.\"},\"limit\":{\"type\":\"integer\",\"description\":\"Maximum rows, capped at 200. Use 0 for the cap. Default 30.\"}");
        sb.Append(",");
        AppendAiToolDefinition(sb, "world_map_route", "Plan an overworld route from the player to a zone or grid cell, preferring roads and avoiding water. Reports step count and how many road/land/shore/water cells the route crosses. By default it also sets the route guide destination so the path is drawn on the world map and in game.", "",
            "\"zone\":{\"type\":\"string\",\"description\":\"Destination zone display name, exact or partial. Provide this, or x and y.\"},\"x\":{\"type\":\"integer\",\"description\":\"Destination world grid X. Required when zone is omitted.\"},\"y\":{\"type\":\"integer\",\"description\":\"Destination world grid Y. Required when zone is omitted.\"},\"set_destination\":{\"type\":\"boolean\",\"description\":\"Also set the route guide destination and enable the route display. Default true.\"}");
        sb.Append(",");
        AppendAiToolDefinition(sb, "query_player_state", "Read a snapshot of the player: level, HP/mana/stamina, faith, feat points, currency, current zone and danger level, strongest elements, party size and home base level plus resident count. Use this before giving build or progression advice.", "",
            "");
        sb.Append(",");
